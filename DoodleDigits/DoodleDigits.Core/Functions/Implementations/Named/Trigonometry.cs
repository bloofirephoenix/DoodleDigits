...
public static partial class NamedFunctions {
    ...
    private static Rational RadianToDegreesConstant = 360 / RationalUtils.Tau;
    private static Rational DegreesToRadianConstant = RationalUtils.Tau / 360;

    /// <summary>
    /// Converts radians to degrees if context is degrees
    /// </summary>
    /// <param name="value"></param>
    /// <param name="context"></param>
    private static void ApplyRadiansToDegrees(ref Value value, ExecutionContext context) {
        if (value is not RealValue realValue) {
            return;
        }
        ApplyRadiansToDegrees(ref realValue, context);
        value = realValue;
    }

    /// <inheritdoc cref="ApplyRadiansToDegrees(ref Value, ExecutionContext)" />
    private static void ApplyRadiansToDegrees(ref RealValue value, ExecutionContext context) {
        if (context.AngleUnit == AngleUnit.Radians) {
            return;
        }
        value = value.Clone(value: value.Value * RadianToDegreesConstant);
    }

    /// <summary>
    /// Converts degrees to radians if context is degrees
    /// </summary>
    /// <param name="value"></param>
    /// <param name="context"></param>
    private static void ApplyDegreesToRadians(ref Value value, ExecutionContext context) {
        if (value is not RealValue realValue) {
            return;
        }
        ApplyDegreesToRadians(ref realValue, context);
        value = realValue;
    }

    private static void ApplyDegreesToRadians(ref RealValue value, ExecutionContext context) {
        if (context.AngleUnit == AngleUnit.Radians) {
            return;
        }
        value = value.Clone(value: value.Value * DegreesToRadianConstant);
    }
    ...
    public static Value ArcSine(Value value, ExecutionContext<Function> context) {
        ...
        if (realValue.Value == Rational.Zero) {
            returnValue = realValue.Clone(value: Rational.Zero);
        } else if (realValue.Value == Rational.One) {
            returnValue = realValue.Clone(value: TauFourth);
        } else {
            returnValue = RealValue.FromDouble(Math.Asin((double)realValue.Value), false, realValue.Form, context.Node);
        }

        ApplyRadiansToDegrees(ref returnValue, context);
        return returnValue;
    }
    ...
    public static Value ArcCosine(Value value, ExecutionContext<Function> context) {
        ...
        if (realValue.Value == Rational.Zero) {
            ...
            returnValue = RealValue.FromDouble(Math.Acos((double)realValue.Value), false, realValue.Form, context.Node);
        }
    ...
    public static Value ArcTangent(Value value, ExecutionContext<Function> context) {
        ...
        var returnValue = RealValue.FromDouble(Math.Atan((double)realValue.Value), false, realValue.Form, context.Node);
        ApplyRadiansToDegrees(ref returnValue, context);
        return returnValue;
    }
