...
static class ExecutionTestUtils {
    ...
    public static void AssertEqual(Rational expected, string input, CalculatorSettings? settings = default) {
        AssertEqual(new RealValue(expected), input, settings);
    }

    public static void AssertEqual(bool expected, string input, CalculatorSettings? settings = default) {
        AssertEqual(new BooleanValue(expected), input, settings);
    }

    public static void AssertEqual(Value expected, string input, CalculatorSettings? settings = default) {
    ...
    public static CalculationResult CalculateString(string input, CalculatorSettings? settings = default) {
        Calculator calculator = new(settings ?? new());
